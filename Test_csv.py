...
parser.add_argument('--netG', default='D:/PhD_Study/Code/PF-Net-Point-Fractal-Network/Trained_Model/point_netG50.pth', help="path to netG (to continue training)")
...
# 输入的数据
opt.infile = 'test_one/crop4-1.csv'
opt.infile_real = 'test_one/real4-1.csv'
...
real2_idx = utils.farthest_point_sample(real,256, RAN = False)
real2 = utils.index_points(real,real2_idx)
real3_idx = utils.farthest_point_sample(real,512, RAN = True)
...
save_crop_path = opt.infile[0:-4] + '_Net.txt'
save_fake_path = opt.infile[0:-4] + '_fake.txt'
# np.savetxt('test_one/crop_ours'+'.csv', np_crop, fmt = "%f,%f,%f")
# np.savetxt('test_one/fake_ours'+'.csv', np_fake, fmt = "%f,%f,%f")
np.savetxt(save_crop_path, np_crop, fmt = "%f,%f,%f")
np.savetxt(save_fake_path, np_fake, fmt = "%f,%f,%f")
