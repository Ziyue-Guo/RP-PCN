...
dset = RP_PCN_loader.PartDataset( root='../dataset/mydata',classification=True, class_choice=None, npoints=opt.pnum, split='train', crop_point_num=opt.crop_point_num)
assert dset
...
test_dset = RP_PCN_loader.PartDataset( root='../dataset/mydata',classification=True, class_choice=None, npoints=opt.pnum, split='test', crop_point_num=opt.crop_point_num)
...
if opt.D_choose == 1:
    for epoch in range(resume_epoch,opt.niter):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            f=open('D:/PhD_Study/Code/PF-Net-Point-Fractal-Network/Checkpoint/loss_PFNet.txt','a')
        ...
        if errG < best_errG:
            ...
            torch.save({'epoch':epoch+1,
                    ...
                    '../Trained_Model/best_point_netG.pth' )
            torch.save({'epoch':epoch+1,
                    'state_dict':point_netD.state_dict()},
                    '../Trained_Model/best_point_netD.pth' )
    ...
    for epoch in range(resume_epoch,opt.niter):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            print('[%d/%d][%d/%d] Loss_G: %.4f / %.4f '
                  % (epoch, opt.niter, i, len(dataloader), 
                      errG_l2,CD_LOSS))
            f=open('../Checkpoint/loss_PFNet.txt','a')
