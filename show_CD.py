...
parser.add_argument('--netG', default='../Trained_Model/point_netG50.pth', help="path to netG (to continue training)")
parser.add_argument('--netD', default='', help="path to netD (to continue training)")
parser.add_argument('--manualSeed', type=int, help='manual seed')
parser.add_argument('--drop',type=float,default=0.2)
parser.add_argument('--num_scales',type=int,default=3,help='number of scales')
parser.add_argument('--point_scales_list',type=list,default=[3072,2048,1024],help='number of points in each scales')
...
test_dset = RP_PCN_loader.PartDataset( root='../dataset/mydata',classification=True, class_choice=None, npoints=opt.pnum, split='test')
...
for i, data in enumerate(test_dataloader, 0):
    ...
    input_cropped1 = torch.FloatTensor(opt.batchSize, 1, opt.pnum - opt.crop_point_num, 3)
    p_origin = [0,0,0]

    # choice = [torch.Tensor([1,0,0]),torch.Tensor([0,0,1]),torch.Tensor([1,0,1]),torch.Tensor([-1,0,0]),torch.Tensor([-1,1,0])]
    choice = [torch.Tensor([0,0,0])]
    index = random.sample(choice,1)
    distance_list = []
    p_center = index[0]
    for num in range(opt.pnum):
        distance_list.append((distance_squre(real_point[0, 0, num], p_center), num))
    # 根据距离排序，只取最近的几个点进行删除
    distance_order = sorted(distance_list, key=lambda x: x[0])
    indices_to_remove = [idx for _, idx in distance_order[:opt.crop_point_num]]
                    
    # 构建一个保留点的索引
    all_indices = set(range(opt.pnum))
    indices_to_keep = list(all_indices - set(indices_to_remove))
    indices_to_keep = sorted(indices_to_keep)  # 确保索引是有序的
    
    for sp, idx in enumerate(indices_to_remove):
        real_center[0, 0, sp] = real_point[0, 0, idx]

    for sp, idx in enumerate(indices_to_keep):
        input_cropped1[0, 0, sp] = real_point[0, 0, idx]
